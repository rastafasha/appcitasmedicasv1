...
export class EditPatientMComponent {
  ...
  showUser() {
    this.patientService.getPatient(this.patient_id).subscribe((resp: any) => {
      ...
      this.birth_date = this.patient_selected.birth_date
        ? new Date(this.patient_selected.birth_date).toISOString()
        : "";
  ...
  save() {
    ...
    this.patientService
      .editPatient(formData, this.patient_id)
      .subscribe((resp: any) => {
        if (resp.message == 403) {
